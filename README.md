![Chris Quinn's GitHub Profile, Header Image](http://i.imgur.com/7lnmioa.jpg)

## About

Full Stack Developer @ [Gravitywell](https://gravitywell.co.uk)

![](http://i.imgur.com/wWzX9uB.png) [chrisquinnr](https://twitter.com/chrisquinnr)

### I architect, design and build awesome web apps in Node.js, PHP & more.

* Node.js, Meteor.js, MongoDB, LAMP, JS, CSS, HTML, JQuery
* Cucumber, Jasmine, Mocha, (BDD), nodeunit, Chimp.js
* AWS, Plesk, Vagrant, CentOS, *nix
* Wordpress, Drupal, Salesforce
* Python, Ruby on Rails, Adobe Creative Cloud


## Articles
 
* [Pretty Wonderful Advancement](https://www.createmarketing.co.uk/blog/progressive-web-apps/)

   Fresh from a Google PWA bootcamp, notes on the potential of [PWA tech](https://developers.google.com/web/progressive-web-apps/)

* [NoCoffee](https://medium.com/@chrisquinnr/nocoffee-the-vision-impairment-simulator-you-should-be-using-f81992c1effc)

  The vision impairment simulator we should all be using.
 
* [Serverless, do more](https://www.gravitywell.co.uk/latest/labs/posts/serverless-do-more/)

   Intro & discussion of microservice architectures using [Serverless](https://serverless.com/)

* [Pelico Dashboard](https://www.gravitywell.co.uk/latest/rd/posts/pelico-project-dashboard/)

   Developing a dashboard & courier management tool for a local food delivery startup.

* [Hackvember](https://www.gravitywell.co.uk/latest/hackathon/posts/hackvember-2016-join-us/) & [Zombie Chess](https://www.gravitywell.co.uk/latest/design/posts/hackvember-2016/)

   Inviting collaborators for our entry into the [GitHub 2016 GameOff](https://github.com/github/game-off-2016), picked as [one of the highlights of the jam by Github](https://github.com/blog/2292-game-off-iv-highlights).

* [Hawk](https://www.gravitywell.co.uk/latest/design/posts/hawk/)

   A look at the tech behind our 2016 week long hackathon project to build a natural language e-commerce interface.

* [Into to Meteor](https://www.gravitywell.co.uk/latest/how-to/posts/meteor-bring-the-awesome-part-0/)

   Introduction to Meteor.js.
